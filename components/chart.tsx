    ...
    const res = await postData(url)
      .then((data) => {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          const ddate = year + '-' + month + '-' + day + ' ' + hours + ':' + minutes
          if (time === '1y' || time === '1m' || time === '6m' || time === '5y' || time === '10y' || time === 'max') {
            yResult.push(ydata)
          }
          if (time === '1d' || time === '1w') {
            yResult.push(ddate)
      ...
      [
        {
          name: 'EUR',
          data: price,
          // [0.05, 1.81, -0.41, -0.78, 1, -1.24, 3.44, -0.41, 3.51, -0.34, 0.99, 1.28, 0.03],
          type: 'line',
        },
        {
          name: 'change',
        ...
        <div>
          <ul>
            <li onClick={() => handleClick('1d')} >
              <span className={isActive === '1d' ? 'active' : 'inactive'}> 1 天</span>
            </li>
            <li onClick={() => handleClick('1w')}>
              <span className={isActive === '1w' ? 'active' : 'inactive'}>1 周</span>
            </li>
            <li onClick={() => handleClick('1m')}>
              <span className={isActive === '1m' ? 'active' : 'inactive'}>1 月</span>
            </li>
            <li onClick={() => handleClick('6m')}>
              <span className={isActive === '6m' ? 'active' : 'inactive'}>半 年</span>
            </li>
            <li onClick={() => handleClick('1y')}>
              <span className={isActive === '1y' ? 'active' : 'inactive'}>1 年</span>
            </li>
            <li onClick={() => handleClick('5y')}>
              <span className={isActive === '5y' ? 'active' : 'inactive'}>5 年</span>
            </li>
            <li onClick={() => handleClick('10y')}>
              <span className={isActive === '10y' ? 'active' : 'inactive'}>10 年</span>
            </li>
            <li onClick={() => handleClick('max')}>
              <span className={isActive === 'max' ? 'active' : 'inactive'}>全 部</span>
            </li>
          </ul>
        </div>
      </div>
